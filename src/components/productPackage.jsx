    ...
    return this.props.productPackage.map((item) => {
      return (
        <Card style={styles.card} key={item.id_product_package}>
          ...
          <CardActions style={styles.linkpackage}>
            <Link
              to={`/productPackageDetails?${item.id_product_package}`}
              style={styles.link}
            >
              <Fab
                variant="extended"
  ...
  render() {
    return (
      <div style={styles.root}>
        ...
        <div style={styles.cardContainer}>{this.renderProductPackage()}</div>
      </div>
    );
  }
}
...
export default connect(mapStateToProps, { getAllProductPackage })(
  ProductPackage
);
