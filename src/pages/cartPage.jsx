...
class CartPage extends React.Component {
  ...
  calculateAmountProduct = async () => {
    ...
    await this.props.resultPcs.map((item, index) =>
      this.setState({
  ...
  calculateAmountPackage = async () => {
    ...
    await this.props.resultPkg.map((item, index) =>
      this.setState({
  ...
  handleEditConfirm = async (item) => {
    if (
      ...
      !this.state.edited_total_modal ||
      !this.state.edited_total_sell
    )
      return console.log("quantity is empty");
  ...
  handleDeletePkg = async (item) => {
    ...
    Axios.delete(
      URL + "/deletepkg/" + order_number + "/" + package_id + "/" + package_no
    );
    await this.props.getCartUser(localStorage.getItem("id"));
    await this.calculateAmountPackage();
    await this.calculateAmount();
  ...
  handleChangePayment = async (event) => {
    ...
    await console.log(
      "selected payment type id : ",
      this.state.selected_payment_type_id
    );
  };
        ...
        return (
          <TableRow key={index}>
            ...
            <TableCell align="center">
              <Button onClick={() => this.handleEditMinus(item)} size="small">
                -
              </Button>
              {this.state.edited_product_qty}
              <Button onClick={() => this.handleEditPlus(item)} size="small">
                +
              </Button>
            </TableCell>
                ...
                {this.state.paymentMethod[0] ? (
                  <FormControl style={{ minWidth: 200, backgroundColor: "" }}>
                    ...
                    >
                      ...
                      <MenuItem
                        value={this.state.paymentMethod[0].id_payment_type}
                      >
                        {this.state.paymentMethod[0].via_bank}
                      </MenuItem>
                      <MenuItem
                        value={this.state.paymentMethod[1].id_payment_type}
                      >
                        {this.state.paymentMethod[1].via_bank}
                      </MenuItem>
                      <MenuItem
                        value={this.state.paymentMethod[2].id_payment_type}
                      >
                        {this.state.paymentMethod[2].via_bank}
                      </MenuItem>
                    </Select>
                  </FormControl>
                ) : null}
  ...
  render() {
    ...
    if (this.state.redirectToHistoryTransaction)
      return <Redirect to="/historyUser" />;
      ...
      <div style={styles.root}>
        <Paper style={styles.rootContainer} elevation={10}>
          <div style={styles.header}>
            ...
            >{`Total IDR ${this.state.amount.toLocaleString()}.00`}</Typography>
            <Divider />
            <Button
          ...
          {this.props.resultPcs[0] ? (
            <Table style={{ marginBottom: 20 }}>
              <TableHead style={{ backgroundColor: "#cbe2d6" }}>
